...
## Startup
The login logic is weird due to quirks with the raspberry pi zero. After investigation data has to be sent on a separate serial connection to the startup. During startup, the pi is logged in and pi_sender.py is launched. The Pi Camera gain is then sent which then triggers the start of data being sent. If data is already being sent from the Pi (due to it already being logged in and running), the startup is skipped.

## GUI
The GUI plots data, has functionality to find peaks and smooths the data. You can save a spectrum using the save button and entering a filename.
